...
func (s *Solution) AddOperation(operation *Operation) {
	...
	if operation.Operation == solution.Noop || operation.Operation == solution.Install {
		s.names = append(s.names, operation.Package.Name)
	}

	switch operation.Operation {
	case solution.Install, solution.Noop:
...
func (s *Solution) Get(name string) *ops.Header {
	for index := range s.operations {
		if s.operations[index].Package.Name == name && s.operations[index].Operation != solution.Remove {
